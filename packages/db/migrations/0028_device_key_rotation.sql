...
-- Consolida o hardening previamente validado em DEV/STAGE com a rotação segura.
-- A chave bruta é gerada fora do banco; somente SHA-256 é persistido.
...
ALTER TABLE public.device_ingest_keys
  ...
  REFERENCES public.device_ingest_keys(id) ON DELETE RESTRICT;
...
-- Mantém a assinatura histórica da RPC. O estado efetivo é calculado sem expor hash.
...
REVOKE ALL ON FUNCTION public.rotate_device_ingest_key(uuid, uuid, text, text, timestamptz, integer)
  FROM PUBLIC, anonymous, authenticated;
GRANT EXECUTE ON FUNCTION public.rotate_device_ingest_key(uuid, uuid, text, text, timestamptz, integer)
  TO authenticated;
...
REVOKE ALL ON FUNCTION public.revoke_device_ingest_key(uuid)
  FROM PUBLIC, anonymous, authenticated;
GRANT EXECUTE ON FUNCTION public.revoke_device_ingest_key(uuid)
  TO authenticated;

COMMENT ON FUNCTION public.register_device_ingest_key(uuid, text, text, timestamptz)
IS 'Registers only a SHA-256 device-key hash. Expiration defaults to 90 days and cannot exceed 365 days. Raw keys must never be sent to this function.';
...
COMMENT ON FUNCTION public.list_device_ingest_keys(uuid)
IS 'Lists sanitized ingest-key metadata using server-side scope authorization. Raw key/hash are never returned.';
