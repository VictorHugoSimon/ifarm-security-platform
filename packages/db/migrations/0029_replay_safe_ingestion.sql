...
BEGIN
  ...
  -- Serialize state transitions per device. This also makes concurrent replay deterministic.
  ...
  IF v_inserted IS NULL THEN
    ...
    RETURN QUERY
      SELECT false, v_existing_status, v_existing_received, 'duplicate'::text;
    RETURN;
  END IF;

  -- A buffered heartbeat older than already-observed telemetry is historical evidence only.
  ...
  IF p_source_at IS NOT NULL
     AND v_latest_effective IS NOT NULL
     AND p_source_at < v_latest_effective THEN
    RETURN QUERY
  ...
  IF v_old_status IS DISTINCT FROM p_status THEN
    ...
    INSERT INTO public.security_events(
      ...
      v_severity,
      'not_required',
      coalesce(p_source_at, v_received),
      jsonb_build_object(
...
BEGIN
  ...
  END IF;
  IF p_recorded_at IS NULL OR p_recorded_at > now() + interval '10 minutes' THEN
  ...
  -- Serialize state/geofence transitions per asset.
  SELECT a.organization_id, a.property_id, p.neighborhood_id, a.device_id,
    ...
    INTO v_org, v_prop, v_neigh, v_linked_device,
         v_geofence, v_enabled, v_previous, v_last_position
  ...
  END IF;
  IF v_linked_device IS NULL OR v_linked_device <> p_device_id THEN
    RAISE EXCEPTION 'asset_device_mismatch';
  END IF;
  IF NOT EXISTS (
    SELECT 1
      FROM public.device_ingest_keys k
  ...
  UPDATE public.device_ingest_keys
   ...
   WHERE device_id = p_device_id
     AND key_hash = lower(p_key_hash)
     AND status = 'active';

  v_point := ST_SetSRID(ST_MakePoint(p_longitude, p_latitude), 4326)::geography;
  IF v_enabled AND v_geofence IS NOT NULL THEN
    v_inside := ST_Covers(v_geofence, v_point::geometry);
  ELSE
    v_inside := NULL;
  END IF;

  INSERT INTO public.asset_positions(
    organization_id, asset_id, device_id, source_event_id, position,
    recorded_at, received_at, speed_kmh, heading_degrees, accuracy_m,
    inside_geofence, metadata
  ) VALUES (
    v_org, p_asset_id, p_device_id, v_event_id, v_point,
    p_recorded_at, v_received, p_speed_kmh, p_heading_degrees, p_accuracy_m,
    v_inside, coalesce(p_metadata, '{}'::jsonb)
  )
  ...
  IF v_inserted IS NULL THEN
    RETURN QUERY
      SELECT false, ap.inside_geofence, 'duplicate'::text, ap.received_at
        FROM public.asset_positions ap
       WHERE ap.asset_id = p_asset_id
         AND ap.source_event_id = v_event_id
       LIMIT 1;
    RETURN;
  END IF;
  ...
  IF v_last_position IS NULL OR p_recorded_at >= v_last_position THEN
    IF v_enabled AND v_geofence IS NOT NULL THEN
      v_new_state := CASE WHEN v_inside THEN 'inside' ELSE 'outside' END;
    ELSE
      v_new_state := 'unknown';
    END IF;
    ...
    UPDATE public.assets
       SET location = v_point,
           last_position_at = p_recorded_at,
           geofence_state = v_new_state,
           updated_at = now()
     WHERE id = p_asset_id;

    IF v_enabled AND v_geofence IS NOT NULL THEN
      IF v_previous = 'inside' AND v_new_state = 'outside' THEN
        v_transition := 'exit';
        v_event_type := 'asset_geofence_exit';
        v_severity := 'high';
      ELSIF v_previous = 'outside' AND v_new_state = 'inside' THEN
        v_transition := 'return';
        v_event_type := 'asset_geofence_return';
        v_severity := 'informational';
      ELSIF v_previous = 'unknown' AND v_new_state = 'outside' THEN
        v_transition := 'outside_detected';
        v_event_type := 'asset_geofence_outside_detected';
      ...
      IF v_event_type IS NOT NULL THEN
        INSERT INTO public.security_events(
          ...
          v_org, v_neigh, v_prop, p_device_id, p_asset_id,
          v_event_type, v_severity, NULL, 'not_required', p_recorded_at,
          jsonb_build_object(
            'source', 'asset_geofence_rule',
            'source_event_id', v_event_id,
            'latitude', p_latitude,
            'longitude', p_longitude,
            'speedKmh', p_speed_kmh,
            'accuracyM', p_accuracy_m,
            'geofenceState', v_new_state
          )
        );
      END IF;
    END IF;
...
COMMENT ON FUNCTION public.ingest_asset_position(
  uuid, uuid, text, text, timestamptz, double precision, double precision,
  numeric, numeric, numeric, jsonb
