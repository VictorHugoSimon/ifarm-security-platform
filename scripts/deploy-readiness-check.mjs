...
requireInvariant(rootPackage.scripts?.['external-alerts:check'] === 'node scripts/external-alerts-readiness-check.mjs', 'external alerts readiness gate must remain registered.');
...
console.log('Deploy readiness check passed: private-repo gate, isolated Cloudflare credentials, access lifecycle/audit/navigation/access-aware-navigation/access-landing/support/privacy/pilot/auth-provider/admin-bootstrap/admin-lifecycle/backup/evidence-storage/external-alerts gates, HTTP security artifact, pinned Wrangler, Pages contract and smoke gate preserved.');
