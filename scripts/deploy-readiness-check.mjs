...
requireInvariant(rootPackage.scripts?.['asset-position-ordering:check'] === 'node scripts/asset-position-ordering-check.mjs', 'asset position ordering safety gate must remain registered.');
requireInvariant(rootPackage.scripts?.['gps-timestamp-tie:check'] === 'node scripts/gps-timestamp-tie-check.mjs', 'GPS timestamp tie gate must remain registered.');
requireInvariant(rootPackage.scripts?.['telemetry-timing:check'] === 'node scripts/telemetry-timing-observability-check.mjs', 'telemetry timing observability gate must remain registered.');
requireInvariant(rootPackage.scripts?.['deploy-gate-parity:check'] === 'node scripts/deploy-gate-parity-check.mjs', 'deploy gate parity must remain registered.');
...
for (const expected of [
  'IFARM_SECURITY_API_STAGE_WORKER: ifarm-security-api-stage','secrets.IFARM_SECURITY_CLOUDFLARE_API_TOKEN','secrets.IFARM_SECURITY_CLOUDFLARE_ACCOUNT_ID','secrets.IFARM_SECURITY_STAGE_DATABASE_URL','pnpm api-stage:check','pnpm api-security:check','pnpm rate-limit:check','pnpm device-keys:check','pnpm replay:check','pnpm idempotency:check','pnpm ingest-conflict:check','pnpm api-idempotency:check','pnpm heartbeat-ordering:check','pnpm asset-position-ordering:check','pnpm gps-timestamp-tie:check','pnpm telemetry-timing:check','pnpm deploy-gate-parity:check','--config apps/api/wrangler.stage.toml','--secrets-file "$SECRET_FILE"','-D "$HEALTH_HEADERS" "$API_URL/health"','curl --fail --silent --show-error --max-time 20 "$API_URL/ready"','"distributedRateLimitingConfigured":true','"error":"method_not_allowed"'
...
console.log('Deploy readiness check passed: Pages and API STAGE remain isolated behind private-repo, dedicated credentials, distributed rate limiting, replay/idempotency/conflict-response/API-idempotency/heartbeat-ordering/asset-position-ordering/GPS-tie/telemetry-timing protection, security/readiness/device-key gates and smoke checks.');
