...
for (const route of [
  'overview','operations','sos','assets','insurance','community','rural','devices','support','privacy','access','audit','health','events','incidents','evidence','map'
]) {
  requireInvariant(app.includes(`key: '${route}'`), `missing route: ${route}`);
}

for (const component of [
  '<OperationsSOC />','<SOSCenter />','<AssetSecurity />','<InsuranceCenter />','<CommunityCenter />','<RuralStructure />','<DeviceSetup />','<SupportCenter />','<PrivacyRetentionCenter />','<AccessManagement />','<MembershipManagement />','<AccessAuditCenter />','<TelemetrySetup />','<EventCenter />','<IncidentCenter />','<EvidenceVault />','<SecurityMap />'
]) {
